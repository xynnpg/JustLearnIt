...
def admin_panel():
        ...
        if user_id and action:
            ...
            if user:
                if action == 'approve':
                    ...
                    # Delete user's data from storage
                    try:
                        user_storage_path = os.path.join(STORAGE_DIR, 'users', user.email)
                        if os.path.exists(user_storage_path):
                            import shutil
                            shutil.rmtree(user_storage_path)
                    except Exception as e:
                        logger.error(f"Error deleting user data from storage: {e}")
    ...
    for subject_key in SUBJECTS:
        try:
            subject_path = os.path.join(LECTII_DIR, subject_key, 'profesori')
            if os.path.exists(subject_path):
                for professor_email in os.listdir(subject_path):
                    professor_path = os.path.join(subject_path, professor_email)
                    if os.path.isdir(professor_path):
                        for file in os.listdir(professor_path):
    ...
    for subject_key in SUBJECTS:
        try:
            subject_path = os.path.join(TESTE_DIR, subject_key, 'profesori')
            if os.path.exists(subject_path):
                for professor_email in os.listdir(subject_path):
                    professor_path = os.path.join(subject_path, professor_email)
                    if os.path.isdir(professor_path):
        ...
        try:
            ...
            if os.path.exists(subject_path):
                for professor_email in os.listdir(subject_path):
                    professor_path = os.path.join(subject_path, professor_email)
                    if os.path.isdir(professor_path):
                        results_path = os.path.join(professor_path, 'results')
                        if os.path.exists(results_path):
                            for file in os.listdir(results_path):
                                if file.endswith('.json'):
                                    with open(os.path.join(results_path, file), 'r') as f:
                                        grade_data = json.load(f)
                                        # Get student and professor objects
                                        student = User.query.filter_by(email=grade_data.get('student_email')).first()
                                        professor = User.query.filter_by(email=professor_email).first()
                                        
                                        # Create a structured grade object
                                        grade = {
                                            'student': student if student else type('FakeUser', (), {
                                                'name': grade_data.get('student_email', 'Unknown'),
                                                'email': grade_data.get('student_email', 'Unknown')
                                            }),
                                            'professor': professor if professor else type('FakeUser', (), {
                                                'name': professor_email,
                                                'email': professor_email
                                            }),
                                            'subject': subject_key,
                                            'test_title': grade_data.get('lesson_title', 'Unknown'),
                                            'score': grade_data.get('score', 0),
                                            'date': datetime.fromisoformat(grade_data.get('timestamp')) if grade_data.get('timestamp') else datetime.now()
                                        }
                                        grades.append(grade)
        except Exception as e:
    ...
    if action == 'delete_lesson':
        ...
        if lesson_title and subject and professor_email:
            try:
                ...
                lesson_path = os.path.join(LECTII_DIR, subject, 'profesori', professor_email, f"{lesson_title}.html")
                if os.path.exists(lesson_path):
                    os.remove(lesson_path)
                    flash(f'Lesson {lesson_title} has been deleted.', 'success')
                else:
                    flash(f'Lesson {lesson_title} not found.', 'error')
        ...
        if test_title and subject and professor_email:
            try:
                ...
                test_path = os.path.join(TESTE_DIR, subject, 'profesori', professor_email, f"{test_title}.json")
                if os.path.exists(test_path):
                    os.remove(test_path)
                    flash(f'Test {test_title} has been deleted.', 'success')
                else:
                    flash(f'Test {test_title} not found.', 'error')
...
def edit_lesson(subject, lesson_name, email):
    if request.method == 'GET':
        try:
            ...
            lesson_path = os.path.join(LECTII_DIR, subject, 'profesori', email, f"{lesson_name}.html")
            print(f"Fetching lesson from: {lesson_path}")
            ...
            if os.path.exists(lesson_path):
                ...
                print(f"Lesson file not found at: {lesson_path}")
